...
def load_base_model(
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        attn_implementation="sdpa",  # Native PyTorch 2.4 SDPA, no extra installation required
    )
...
def add_skip_token(
    ...
    1. Collect embeddings of semantically related tokens ("skip", "pass", "omit", "therefore")
    ...
    for word in config.skip_semantic_tokens:
        # Tokenization may yield multiple subwords; take the first
    ...
    # Also update lm_head (output layer) if weights are tied
...
def attach_skip_adapter(
    ...
    The Skip Adapter exists as an independent module and does not modify the
    original model structure. It is accessed via model.skip_adapter.
    ...
    """
    # Change 1: drop the bottleneck_ratio argument and enable the full-dimension residual network
    adapter = SkipAdapter(
    ...
    # Change 2: drop the bottleneck_dim reference in the log and report the full dimension instead
...
def get_last_hidden_state_before_norm(
    ...
    """
    Forward pass that returns the last-layer hidden state (before RMSNorm) and logits.
    ...
    Args:
        ...
        inputs_embeds: direct input embeddings (used for the <SKIP> path)
    ...
    Returns:
        ...
        new_past_key_values: the updated KV cache (if use_cache=True)
    """
    ...
    # Manually apply RMSNorm + lm_head to obtain logits
...
def setup_model_for_phase1(config: SPARKConfig):
    ...
    Note: <SKIP> is not enabled during Phase 1; these components are merely installed in advance.
    ...
    # 4. Put the model into training mode
    model.train()
    ...
    # (the model rarely generates it; freezing is just an extra safeguard).
    # Note: since the embedding layer participates in training as a whole, we exclude
    # the <SKIP> gradient via a hook, handled in grpo_trainer.py.
...
def save_checkpoint(
    ...
    # Save the Skip Adapter separately (it is not part of the original model structure)
...
def load_checkpoint(
    ...
    if os.path.exists(adapter_path):
        try:
            ...
            # Catch size mismatches caused by architecture upgrades (e.g. the bottleneck was removed)
            logger.warning("="*60)
