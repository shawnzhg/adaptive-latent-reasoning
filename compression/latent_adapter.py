...
Key improvements:
  1. Residual connection: the Adapter only learns the corrective difference Delta h
  ...
  3. RMSNorm prefix: aligns with the Pre-Norm architecture of Qwen-2.5
  4. SiLU activation: avoids the dead-neuron problem of ReLU
  5. Zero-initialized W_down: output at step 0 = h_last (identity-mapping cold start)
...
class SkipAdapter(nn.Module):
    ...
    Through the residual structure, the Adapter is equivalent to an identity mapping
    (h_last -> h_last) early in training, and gradually learns the optimal space
    translation as training proceeds.

    Args:
        hidden_size: model hidden dimension d (e.g., 3584 for Qwen-2.5-7B, 2048 for 1.5B)
    """

    def __init__(self, hidden_size: int, **kwargs):
        # **kwargs absorbs legacy parameters such as bottleneck_ratio to keep the interface compatible
        super().__init__()
        self.hidden_size = hidden_size

        # RMSNorm prefix (aligns with the Qwen Pre-Norm convention)
    ...
    def forward(self, h_last: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            h_last: hidden state from the last Transformer layer
                    shape: (..., hidden_size)
