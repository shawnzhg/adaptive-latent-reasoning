...
# ============================================================
# Generic dataset loading: automatically decide load_from_disk vs load_dataset
# ============================================================
...
def smart_load_dataset(path: str, split: Optional[str] = None, **kwargs) -> Dataset:
    ...
    Format A: an Arrow directory saved by save_to_disk
    ...
    Format B: a local directory with a loading script (e.g. a gsm8k clone)
        -> Signature: contains a *.py or *.json loading script, requires a config name
    ...
    if not os.path.isdir(path):
        # Not a local directory -> load from Hub
    ...
    # -- Local directory: decide whether it is save_to_disk (Format A) or script-style (Format B) --
    ...
    # Format B: script-style local directory -> load_dataset + auto-guess config name
...
def _pick_split(ds, split: Optional[str]) -> Dataset:
    """Select the appropriate split from a DatasetDict"""
...
# ============================================================
# Answer extraction -- cascade of multiple strategies
# ============================================================
...
def check_answer(prediction: str, ground_truth: str) -> bool:
    """Check whether the prediction is correct (exact + numeric approximation)"""
...
def extract_boxed_answer(solution: str) -> Optional[str]:
    """Extract the \\boxed{...} answer from a solution (handles nested braces)"""
    pattern = r'\\boxed\{'
...
def extract_math_answer(solution: str) -> str:
    """Extract the answer from a MATH solution: prefer \\boxed{}, fall back to ####, then the last line"""
    # Try \boxed{}
...
def load_math(config: DataConfig) -> List[Dict]:
    ...
    # Check whether there is a separate answer field (MATH-500 has both solution and answer)
    ans_field = None
...
def build_prompt(
    ...
    """
    v3_simple format (empirically optimal on Qwen2.5-1.5B):
...
class MathProblemDataset:
    """Merges GSM8K + MATH and provides batch sampling"""
